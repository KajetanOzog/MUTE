...
def build_jobs(records, config, evaluation, prompts, include_quality=False):
    jobs = defaultdict(list)
    judgments = [{} for record in records]

    for index, record in enumerate(records):
        context = record_context(record, config)
        evaluator_names = list(evaluation.tasks[record["task"]])
        if include_quality and "quality_1_5" not in evaluator_names:
            evaluator_names.append("quality_1_5")
...
def judge_run(
    ...
    for input_path in sorted(Path(run_dir).glob("shard_*.jsonl")):
        ...
        jobs, judgments = build_jobs(
            records, config, evaluation, prompts, include_quality
        )
